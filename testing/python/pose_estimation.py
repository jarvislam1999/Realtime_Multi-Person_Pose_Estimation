...
# sys.path.insert(0, '/software/Anaconda3-5.0.1-el7-x86_64/lib/python3.6/site-packages/caffe/python')
import scipy
import PIL.Image
import math
import time
...
def main():
    print("Begin pose estimation process:")

    # Choose image directory here, will change into command argument later
    #test_image = '../sample_image/ski.jpg'
    #test_image = '../sample_image/upper.jpg'
    test_image = '../sample_image/upper2.jpg'
    #test_image = '../sample_image/upper3.jpg'
    #test_image = '../sample_image/full_body1.jpg'
    #test_image = '../sample_image/full_body2.jpg'#test_image = '../sample_image/party1.jpg'
    
    # Read in and show image
    oriImg = cv.imread(test_image) # B,G,R order
    print("Input image shape: ", oriImg.shape)             
    
    # Code to display input image, will comment out for now
    """
    f = plt.imshow(oriImg[:,:,[2,1,0]]) # reorder it before displaying
    plt.show()
    plt.savefig('initial_pic.jpg')
    """

    # Read in parameters and models
    param, model = config_reader()
    print("Printing out predefined parameters:", param)
    print("Printing out model info:", model)
    multiplier = [x * model['boxsize'] / oriImg.shape[0] for x in param['scale_search']]
    print("Multiplier:", multiplier)

    # Setting CAFFE model
    print("Reading trained CAFFE model obtained from getmodel.sh")
    if param['use_gpu']: 
        caffe.set_mode_gpu()
    ...
    # Show heatmaps and PAFS for multiple scale, will comment out for now
    # print("Show heatmap and PAFS for multiple scale:")
    ...
    for m in range(len(multiplier)):
        ...
        heatmap = np.transpose(np.squeeze(net.blobs['Mconv7_stage6_L2'].data), (1,2,0)) # output 1 is heatmaps
        ...
        # visualization, will comment out for now
        """
    ...
    """
    print("Let's have a closer look on those averaged heatmaps and PAFs!")
        ...
        if(nA != 0 and nB != 0):
            ...
            for i in range(nA):
                for j in range(nB):
                    ...
                    #print(len(np.linspace(candA[i][0], candB[j][0], num=mid_num)))
                    startend2 = list(startend)
                    #print(startend2)
                
                    vec_x = np.array([score_mid[int(round(startend2[I][1])), int(round(startend2[I][0])), 0] \
                                      for I in range(len(np.linspace(candA[i][0], candB[j][0], num=mid_num)))])
                    vec_y = np.array([score_mid[int(round(startend2[I][1])), int(round(startend2[I][0])), 1] \
                                      for I in range(len(np.linspace(candA[i][0], candB[j][0], num=mid_num)))])
